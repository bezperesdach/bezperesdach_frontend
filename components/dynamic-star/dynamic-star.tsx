...
import { IDynamicStarProps, IStar } from "../../types/dynamic-star/types";
import { createEmptyStar, emptyStar } from "../../utils/dynamic-star-utils";
import { reducer } from "./components/reducer/reducer";

import styles from "./dynamic-star.module.css";
...
function DynamicStar({
  ...
  const getFullFillColor = (starData: IStar) => (starData.raw !== emptyStar ? fullStarColor : emptyStarColor);

  const calcStarPoints = (centerX: number, centerY: number, innerCircleArms: number, innerRadius: number, outerRadius: number) => {
  ...
  return (
    <div className={styles.dynamic_star_rating} aria-label={`${rating} of 5`}>
      {stars.map((star, index) => (
        <div key={`${id}_${index}`} className={styles.dynamic_star_container}>
          <svg
          ...
          >
            ...
            <defs>
              ...
              <linearGradient id={`${id}_gradient${star.raw}`}>
                <stop id="stop1" offset={star.percent} stopOpacity="1" stopColor={getFullFillColor(star)} />
                <stop id="stop2" offset={star.percent} stopOpacity="0" stopColor={getFullFillColor(star)} />
                <stop id="stop3" offset={star.percent} stopOpacity="1" stopColor={emptyStarColor} />
                <stop id="stop4" offset="100%" stopOpacity="1" stopColor={emptyStarColor} />
              </linearGradient>
            </defs>
          </svg>
        </div>
